...
def calculate_reward(uav, task, target, max_total_voyage, max_total_time):
    # 先检查约束，若不满足则给一个大负奖励
...
def check_constraints(uav, task):
    ...
    if not uav.alive:
        return False
    # 类型约束
    if task.type == 3 and uav.type == 1:
        return False
    if task.type == 2 and uav.type == 1:
        return False
    if task.type == 1 and uav.type == 2:
        return False
    # 弹药需求
